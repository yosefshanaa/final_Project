...
- [ ] **BLOCKER for any multi-sub-game interop match** (found 2026-07-30 by re-running the warm-up with `--games 2`; a 1-sub-game warm-up hides it): reference peers **re-negotiate before every sub-game** and **alternate roles** (odd = natural, even = opposite), while our peer handshakes once per series and holds one role. Sub-game 2 dies with `Opponent never sent its agreement`. Neither is settled by the book, so both are pair-negotiated items — decide whether to support per-sub-game re-handshake + role alternation in our runtime, or to require opponents to disable both. RUNBOOK §3b has the reproduction
